...
(function () {
  ...
  /* ── ambient aurora backdrop ─────────────────────────────
     Four soft color fields drifting on slow, offset loops.
     Pure CSS animation (transform-only, GPU-composited);
     BACKDROP in data.js: enabled on/off, density = intensity. */
  function initAurora() {
    if (typeof BACKDROP !== "undefined" && BACKDROP && BACKDROP.enabled === false) return;
    var el = document.createElement("div");
    el.className = "aurora";
    el.setAttribute("aria-hidden", "true");
    el.innerHTML = '<span class="a1"></span><span class="a2"></span>' +
                   '<span class="a3"></span><span class="a4"></span>';
    var d = (typeof BACKDROP !== "undefined" && BACKDROP && typeof BACKDROP.density === "number") ? BACKDROP.density : 1;
    if (d < 1) el.style.opacity = String(Math.max(0, d));
